...
async function main(prisma: PrismaClient) {
    ...
    const expected_department_employee_information = await test_department_employee(expected_department_information.dept_no);
    let error_count = 0;

    // Clean up step to ensure that the department to be added does not 
    //  already exist in the database.
    await departments.deleteDepartment([expected_department_employee_information.dept_no]);
    ...
    const department_employee = await deptartment_employees.findByEmployeeNoAndDepartmentNo(
    ...
    await employees.deleteEmployees([expected_department_employee_information.emp_no])
    await departments.deleteDepartment([expected_department_employee_information.dept_no]);
