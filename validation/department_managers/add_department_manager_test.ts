...
async function main(prisma: PrismaClient) {
  ...
  const expected_department_manager_information = await test_department_manager(
    expected_department_information.dept_no
  );
  let error_count = 0;
  ...
  const department_manager =
    await department_managers.findByEmployeeNoAndDepartmentNo(
