...
async function main(prisma: PrismaClient) {
    ...
    error_count = error_count + expect(employee?.emp_no, expected_employee_information.emp_no);

    // Birth date validation
    error_count = error_count + expect(employee?.birth_date.toDateString(), expected_employee_information.birth_date.toDateString());

    // First name validation
    error_count = error_count + expect(employee?.first_name, expected_employee_information.first_name);

    // Last name validation
    error_count = error_count + expect(employee?.last_name, expected_employee_information.last_name);

    // Gender validation
    error_count = error_count + expect(employee?.gender, expected_employee_information.gender);

    // Hire date validation
    error_count = error_count + expect(employee?.hire_date.toDateString(), expected_employee_information.hire_date.toDateString());
