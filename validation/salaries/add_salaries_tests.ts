...
async function main(prisma: PrismaClient) {
  ...
  const expected_salary_information = await test_salary(
    new_emp_no,
    42,
    new_salary_date,
    new_salary_date
  ...
  error_count += expect(salary?.emp_no, expected_salary_information.emp_no);

  // Salary amount validation
  error_count += expect(salary?.salary, expected_salary_information.salary);

  // Salary from date validation
  error_count += expect(
    salary?.from_date.toDateString(),
    expected_salary_information.from_date.toDateString()
  );

  // Salary to date validation
  error_count += expect(
    salary?.to_date.toDateString(),
    expected_salary_information.to_date.toDateString()
  );
