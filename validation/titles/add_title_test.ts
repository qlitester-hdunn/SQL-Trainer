import { employees, titles, PrismaClient } from "@prisma/client";
...
async function main(prisma: PrismaClient) {
  ...
  const expected_title_information = await test_title(
    new_emp_no,
    "Engineer",
    new_title_date,
    new_title_date
  ...
  error_count += expect(title?.emp_no, expected_title_information.emp_no);

  // Title validation
  error_count += expect(title?.title, expected_title_information.title);

  // Title from date validation
  error_count += expect(
    title?.from_date.toDateString(),
    expected_title_information.from_date.toDateString()
  );

  // Title to date validation
  error_count += expect(
    title?.to_date?.toDateString(),
    expected_title_information.to_date.toDateString()
  );
